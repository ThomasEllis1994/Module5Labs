...
router.get("/filter", (req, res) => {
    ...
    if (matchingFriends.length > 0) {
        ...
        res.status(404).json({
            error: "No friends matching gender " + filterGender,
        });
    }
});
...
router.get("/:id", (req, res) => {
    ...
    if (findFriend) {
        res.status(200).json({ result: findFriend });
    } else {
        res.status(404).json({
            result: `There is no friend with the id of ${friendId}`,
        });
    }
});

//not working and I dont know why. Just getting result
