...
// axios example
axios.get("http://localhost:3000/store/products").then((response) => {
...
function addProduct(item) {
    ...
    template.querySelector(".card").className =
        "card " + categories.get(item.category);
    ...
    switch (selectedOrder) {
        ...
        case "title_az":
            sortedProducts.sort((p1, p2) =>
                p2.title == p1.title ? 0 : p1.title > p2.title ? 1 : -1
            );
            break;
        case "title_za":
            sortedProducts.sort((p1, p2) =>
                p2.title == p1.title ? 0 : p2.title > p1.title ? 1 : -1
            );
            break;
    }
...
function searchProducts() {
    ...
    let filteredProducts = allProducts.filter(
        (product) =>
            product.title.toLowerCase().indexOf(searchText) >= 0 ||
...
function expandText(e, productId, fullDescription) {
    e.preventDefault();
    document.querySelector("#product" + productId + " .card-text").innerText =
        fullDescription;
}
